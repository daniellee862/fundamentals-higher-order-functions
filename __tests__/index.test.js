...
describe("Higher Order Functions", () => {
  ...
  describe("once", () => {
    it.only("returns a function on first invocation", () => {
      ...
      const firstInvoke = once(restrictedFunc);
      expect(typeof firstInvoke).toBe("function");
    });
    it.only("returns the value of restricted func", () => {
      function restrictedFunc() {
        return "treasure";
      }
      const firstInvoke = once(restrictedFunc);
      expect(firstInvoke()).toEqual(["treasure", 1]);
    });
    it.only("wont invoke restricted func more than once", () => {
      function restrictedFunc() {
        return "treasure";
      }
      const firstInvoke = once(restrictedFunc);
